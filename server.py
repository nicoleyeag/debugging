...
def update_cart():
    ...
    # get key and add value to order
    # session["cart"][item] += 1
    # ^ this returns value at key (item)
    session["order_total"] += get_item_price(item)

    # session["cart"][item] = 1
    # based on how many keys(items) are in the dictionary we will = qty
    ...
    # if key(coffee/tea) exist:
    #     key[value(quantity)] += 1
    # else
    #     add key(coffee/tea)
